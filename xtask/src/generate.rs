...
fn common_header(font_file: &str, util: &str, test_fn: &str) -> String {
    ...
    s.push_str(&format!("// From samples/{font_file}. Do not edit by hand.\n\n"));
...
fn emit_info_test(src: &Path, prefix: &str, font_file: &str, sf: &SoundFont) {
    ...
    s.push_str("\n}\n");
    fs::write(src.join(format!("{prefix}_info_test.rs")), &s).unwrap();
}
...
fn emit_sample_test(src: &Path, prefix: &str, font_file: &str, sf: &SoundFont) {
    ...
    s.push_str(&format!("    assert_eq!(sf.get_sample_headers().len(), {n});\n\n"));
    s.push_str(&format!(
    ...
    s.push_str("    }\n");
    s.push_str("}\n");
    fs::write(src.join(format!("{prefix}_sample_test.rs")), &s).unwrap();
}
...
fn emit_instrument_test(src: &Path, prefix: &str, font_file: &str, sf: &SoundFont) {
    ...
    s.push_str("    }\n");
    s.push_str("}\n");
    fs::write(src.join(format!("{prefix}_instrument_test.rs")), &s).unwrap();
}
...
fn emit_preset_test(src: &Path, prefix: &str, font_file: &str, sf: &SoundFont) {
    ...
    s.push_str("    }\n");
    s.push_str("}\n");
    fs::write(src.join(format!("{prefix}_preset_test.rs")), &s).unwrap();
}
...
fn process(font_file: &str, prefix: &str) {
    ...
    write_f64_rows(&data_dir.join(format!("{prefix}_instrument_regions.bin")), &rows);
    ...
    write_f64_rows(&data_dir.join(format!("{prefix}_preset_regions.bin")), &rows);
    ...
    println!("{prefix}: samples={} instruments={} presets={}", sample_headers.len(), instruments.len(), presets.len());
}
